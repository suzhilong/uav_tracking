...
bool takeOffTask(float height)
{
    ...
    // std::cout << std::to_string(desired_position[0]) << std::endl;
    // std::cout << std::to_string(desired_position[1]) << std::endl;
    // std::cout << std::to_string(desired_position[2]) << std::endl;
    if (reachTargetPosition(desired_position, MAX_ERROR))
        return true;
...
int main(int argc, char** argv) {
    ...
    ros::init(argc, argv, "Video_Record");
    ros::NodeHandle nh;
    ...
    { 
        if(flag_gps_initialized_OK && !flag_take_off_OK)
        {
            ...
            flag_take_off_OK = takeOffTask(HEIGHT);
        }
